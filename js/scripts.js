...
$(function () {
	...
	*/
	...
	$('.chatbot_link').on('click', '.lnk.discover', function(){
		$('.top-menu a[href="#chatbot-card"]').trigger('click');
	});
	...
	*/
	...
	$('.filter-button-group').on( 'click', '.f_btn1', function() {
		alert("e");
	});
		...
		iframe: {
			patterns: {
				youtube_short: {
				  index: 'youtu.be/',
				  id: 'youtu.be/',
				  src: 'https://www.youtube.com/embed/-_cfWPFxKO0'
				}
			}
		},
		removalDelay: 160,
...
function updateClipboard(newClip) {
	navigator.clipboard.writeText(newClip).then(function() {
		...
		$("#copytext").show()
		setTimeout(function() { $("#copytext").hide(); }, 3000);
	}, function() {
		/* clipboard write failed */
	});
}

var awardSortState="relevant"
function sortAwards(sort="relevance") {
$("#a1").hide();
$("#a2").hide();
$("#a3").hide();
$("#a4").hide();
if (sort==="relevance" && awardSortState!="relevance"){
	$("#a3").insertAfter("#a2");
	$("#a4").insertAfter("#a3");
	awardSortState="relevance";
	}
else if (sort!="relevance" && awardSortState!="recent"){
	$("#a3").insertBefore("#a1")
	$("#a4").insertBefore("#a3")
	awardSortState="recent";
	}
setTimeout(function() { $("#a1").slideDown(599);$("#a2").slideDown(600);$("#a3").slideDown(601);$("#a4").slideDown(601); }, 100);
}


function toggleVisibility(div_id,text_id,behaviour="smooth",scroll=1,updelay=750,text="") {
	...
	if (div_element.style.display === "none") {
	  if (behaviour!="none"){
		  } if (scroll===0 &&behaviour==="smooth"){
		  setTimeout(function() { $("#"+div_id).slideDown(750);})}
		  else{
			$("#"+div_id).show();
		  	setTimeout(function() { div_element.scrollIntoView({behavior: "smooth",block:"start"}); }, 100);}
	  $("#"+text_id).attr('data-back', text+"▲");
	} else {
		setTimeout(function() { $("#"+div_id).slideUp(updelay);})
	  $("#"+text_id).attr('data-back', text+"▼");
	}
}
...
function toggleDarkMode() {
	if (toggleCounter===1){
	if (darkmode===1){
	cssLink="css/new-skin.css"
	darkmode=0
	} else {
	cssLink="css/dark.css"
	darkmode=1;
	}

	var oldlink = document.getElementsByTagName("link").item(12);
